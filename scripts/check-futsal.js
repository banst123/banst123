...
const TARGET_WEEKDAYS = [1, 4, 5]; // 모니터링 대상 요일: 월(1), 목(4), 금(5)
...
async function checkPageForSlots(page, url, logPrefix) {
  // 데이터 누락 및 무한 대기 방지를 위해 networkidle2 스펙 유지
  await page.goto(url, {
  ...
  // 🔍 [요구사항 반영] 1시간 단위 단일 슬롯 발견 상태를 가감 없이 실시간 실시간 로그로 출력
...
async function main() {
  ...
  // 현재 컴퓨터가 담당할 1개 구장에 대한 날짜별 쿼리 리스트 생성 (총 12개 쿼리)
  ...
  // GitHub Actions 2-Core 인프라 사양에 맞춰 안정적인 4개 단위 탭으로 동시 스크랩 진행
  const CHUNK_SIZE = 4; 
  ...
  if (alerts.length > 0) {
    ...
    const lines = [
      `▣ 백운포 [${TARGET_PLACE_NAME}] 예약 가능 알림 (연속 2시간 확보 완료) ▣`,
      `[문자 발송 대상 타임 목록]`,
      ""
    ];
    ...
    available = false;
    message = `현재 [${TARGET_PLACE_NAME}]에 조건(14~16회차 중 연속 2시간 이상)에 맞는 예약 가능 슬롯이 없습니다.`;
  }

  console.log("\n==================================================");
  console.log(`📬 [${TARGET_PLACE_NAME} 최종 알림 내역 요약 - 문자 발송용]`);
  console.log("==================================================");
  console.log(message);

  // GitHub Actions가 감지할 수 있도록 Output 데이터 저장
